...
class vk():
  def __init__(self):
    ...
    """ Количество записей полученных в последнем запросе """    
    self.__counts_records = None
    
    """ Блокировщик, используется в многопоточной реализации execute, 
      для защиты массива в который 'собираются' все данные полученные
  ...
  def fork_execute(self, method_name, list_keys, in_arguments, output_array, process_bar):
    """
      ...
      аккумулятора полученных данных в нескольких потоках.
      method_name -- 
      list_keys -- 
      in_arguments --    
      process_bar -- 
    ...
    if str(type(response)) == "<type 'list'>":    
      ...
      """ 
        в случае если не задан список возвращаемых данных (полей),
  ...
  def get_count(self):
    """
      ...
      Уточнение: Количество найденных и возвращённых записей в большенстве случаев не совпадают.
...
\
  ...
  def numeric_list_to_str_list(in_list):
    """
      Преобразует список целочисленных значений в список их строковых представлений.
